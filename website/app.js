...
(function () {
  ...
  var FR = {
    ...
    'tour.lede': 'Onze étapes, dans l’ordre, chacune s’appuyant sur la précédente. Les étapes 1 à 3 installent jigger et le branchent ; à partir de la 4, on n’installe plus rien — on tape, et on regarde. Toutes les commandes ci-dessous suivent le système choisi dans l’en-tête, et les enregistrements sont ce qui se passe vraiment, pas des maquettes.',
    'tour.note': 'Les deux voies d’entrée sont ici. <strong>Vous gardez vos propres commandes</strong> — <span class="mono">brew</span>, <span class="mono">winget</span>, <span class="mono">pacman</span> — et la fenêtre apparaît par-dessus, sans être invitée (étapes 4 à 7). Ou vous employez <strong>une seule syntaxe pour tous</strong>, <span class="mono">jg</span> (étapes 8 et 9). Aucune ne remplace l’autre, et l’étape 11 montre le binaire répondant sans aucun shell.',
    'tour.showing': 'Parcours suivi pour',
    'tour.s1.h2': 'Installer le binaire.',
    'tour.s1.lede': 'Une commande sur macOS et sur Windows, où le projet publie un paquet. Sur Arch il n’en existe pas, alors Go le construit — jigger n’a aucune dépendance d’exécution en dehors des gestionnaires eux-mêmes, et le binaire compilé est tout ce qu’il y a.',
    'tour.s1.more': 'Pourquoi ces URL, ce que construit la formule, comment mettre à jour ensuite : la page <a href="/utiliser.html#telecharger">L’utiliser</a> porte le détail. Ce parcours s’en tient au chemin le plus court.',
    'tour.s2.h2': 'Le brancher dans votre shell.',
    'tour.s2.lede': 'Le binaire seul ne complète rien. Ce qui dessine la fenêtre et surveille votre frappe est un greffon de shell, et il faut que votre shell le charge — une fois, dans son fichier de démarrage.',
    'tour.s2.mac': 'Une ligne dans <span class="mono">~/.zshrc</span>, puis <span class="mono">exec zsh</span>. Elle couvre tous les gestionnaires présents sur la machine — le greffon n’a pas besoin qu’on lui dise lequel.',
    'tour.s2.win': 'Le bucket n’a installé que le <strong>binaire</strong>. Le module vient du dépôt : clonez-le d’abord, puis importez-le depuis <span class="mono">$PROFILE</span> et ouvrez un nouvel onglet.',
    'tour.s2.omarchy': 'Le greffon n’est pas dans le module Go — clonez le dépôt, puis chargez-le depuis <span class="mono">~/.zshrc</span> et lancez <span class="mono">exec zsh</span>.',
    'tour.s3.h2': 'Vérifier que c’est pris.',
    'tour.s3.lede': 'La version d’abord, puis qui répond. Un vieux binaire qui masque une installation plus récente dans le <span class="mono">PATH</span> est la panne la plus pénible à diagnostiquer, et une ligne tranche.',
    'tour.s3.note': 'Rien du tout ? Le greffon le dit quand il refuse de se charger : un message, au démarrage du shell, signale que le binaire est absent du <span class="mono">PATH</span>, ou qu’il est trop ancien pour ce greffon. Il n’échoue jamais en silence.',
    'tour.s4.h2': 'Votre propre commande, interceptée.',
    'tour.s4.lede': 'C’est le moment qu’il faut comprendre. Vous n’apprenez <strong>aucune</strong> commande nouvelle : vous tapez celle que vous tapiez déjà, et le cadre apparaît seul sous votre invite, se resserrant à chaque lettre. Rien à déclencher, rien à retenir — et si vous l’ignorez, votre ligne part exactement comme avant.',
    'tour.s4.try': 'Ouvrez un shell neuf et tapez la ligne ci-dessous <strong>sans appuyer sur Entrée</strong>.',
    'tour.demo.mac01': 'brew install fire — le cadre arrive tout seul et se resserre à chaque lettre. Pris sur macOS.',
    'tour.demo.win01': 'winget install fire — même cadre, mêmes touches, un autre catalogue : winget répond avec des identifiants Publisher.Package là où brew affiche des noms de formules nus. Pris sur Windows.',
    'tour.demo.omarchy01': 'yay -S visual-studio — les dépôts et l’AUR dans une seule liste : ◆ un paquet des dépôts, ▣ un paquet de l’AUR, ● un déjà installé. Pris sur Omarchy.',
    'tour.s5.h2': 'Choisir, et insérer.',
    'tour.s5.lede': 'Cinq touches, et elles sont les mêmes partout — quel que soit le gestionnaire qui répond, et sur les trois systèmes.',
    'tour.k1': 'insère le candidat courant',
    'tour.k2': 'entre dans la liste, puis descend d’un candidat',
    'tour.k3': 'remonte ; au premier candidat, rend le clavier au shell',
    'tour.k4': 'complète la dernière partie et lance la ligne, d’une seule frappe',
    'tour.k5': 'referme la fenêtre pour cette ligne — ⇥ la rouvre',
    'tour.s5.arrows': '<strong>Vos flèches ne sont jamais confisquées.</strong> Tant que la fenêtre n’a pas le clavier, <kbd>↑</kbd> et <kbd>↓</kbd> restent l’historique de votre shell, fenêtre ouverte ou non. Ce qu’elles feront est écrit dans le pied du cadre : vous n’avez jamais à deviner dans quel mode vous êtes.',
    'tour.s6.h2': 'Filtrer plus fin : <kbd>^R</kbd>.',
    'tour.s6.lede': 'Le filtre par préfixe s’épuise dès qu’on ne sait pas comment le nom commence. <kbd>^R</kbd> bascule le filtre — et rien que le filtre : la ligne, le cadre et les touches ne bougent pas, et le titre indique le mode en cours.',
    'tour.s6.how': 'Regardez la liste <em>grandir</em> au moment de la bascule : le motif n’est pas ancré, il correspond n’importe où dans un nom. Puis l’alternance la ramène à quatre — ce qu’aucune recherche par préfixe ne sait exprimer. Un motif qui ne compile pas ne retient rien, et le cadre le dit plutôt que de déverser le catalogue entier.',
    'tour.demo.mac04': 'fire par préfixe, puis ^R — arrayfire rejoint la liste, parce que le motif n’est pas ancré. Puis (bird|fly) n’en garde que quatre. Pris sur macOS.',
    'tour.demo.win04': 'winget install fire propose vingt et un candidats par préfixe. ^R, puis (bird|blade), n’en garde que quatre. Pris sur Windows.',
    'tour.demo.omarchy04': 'Le même élargissement, sur Arch : fire par préfixe, puis ^R et les noms qui portent fire au milieu s’ajoutent. (bird|fly) ne garde ensuite que les firebird et les firefly, dépôts et AUR à la fois. Pris sur Omarchy.',
    'tour.s7.h2': 'Compléter, puis exécuter — pour de vrai.',
    'tour.s7.lede': '<kbd>⏎</kbd> complète la dernière partie <strong>et</strong> lance la ligne, d’une seule frappe. À partir de là, jigger ne fait plus rien du tout : ce qui défile est la sortie du gestionnaire, relayée telle quelle — barres de progression, invites et élévation comprises.',
    'tour.demo.mac05': 'L’installation par la fenêtre se comporte pareil sur macOS, avec brew qui répond. Il n’y a pas d’enregistrement, et il n’y en aura pas : il installerait des paquets pour de vrai sur la machine qui filme. Basculez sur Windows ci-dessus — il tourne sur une machine virtuelle faite pour ça, et le geste est identique.',
    'tour.demo.win05': 'jg install hexy, complété en hexyl, puis lancé — la sortie de scoop, relayée sans y toucher. Pris sur Windows, contre un vrai gestionnaire.',
    'tour.demo.omarchy05': 'Pareil sur Arch, avec yay qui répond — ses propres questions de l’AUR relayées telles quelles, puisque jigger ne fait plus rien une fois la ligne lancée. Il n’y a pas de capture, et il n’y en aura pas : en filmer une demande d’installer pour de vrai, ce qui relève d’une machine dédiée. C’est exactement ce que fait l’enregistrement Windows.',
    'tour.s8.h2': 'L’autre voie : une seule syntaxe, <span class="mono">jg</span>.',
    'tour.s8.lede': 'Tout ce qui précède gardait les mots de votre gestionnaire. <span class="mono">jg</span> en parle une seule pour tous : <span class="mono">jg install fd</span> atteint celui qui connaît <span class="mono">fd</span>, sans que vous ayez à savoir lequel. La façade ne fait qu’ajouter — <span class="mono">brew install fd</span> continue de marcher exactement comme avant, fenêtre comprise.',
    'tour.s8.route': 'Quand plusieurs gestionnaires connaissent le même nom, jigger ne choisit <strong>jamais</strong> à votre place : le sélecteur s’ouvre et vous tranchez. Quand aucun ne le connaît, il le dit, avec les voisins les plus proches.',
    'tour.demo.mac02': 'jg install fd — la façade répond pour le gestionnaire qui connaît le paquet. Pris sur macOS.',
    'tour.demo.win02': 'jg install node — avec deux gestionnaires côte à côte, la colonne de droite nomme celui qui répond pour chaque candidat. Pris sur Windows.',
    'tour.demo.omarchy02': 'jg install fd, avec un détail qui appartient à Arch : pacman et yay sont deux portes sur la même base, aussi la façade liste-t-elle vos paquets <strong>une</strong> fois, jamais deux. Pris sur Omarchy.',
    'tour.s9.h2': 'Même geste, d’autres verbes.',
    'tour.s9.lede': 'Rien ici n’est propre à l’installation. <span class="mono">upgrade</span>, <span class="mono">search</span>, <span class="mono">list</span> et <span class="mono">outdated</span> se complètent et se lancent de la même façon — douze verbes en tout, chacun traduit dans ce que votre gestionnaire attend vraiment.',
    'tour.demo.mac06': 'C’est pareil sur macOS, où c’est brew qui met à jour — et pas de capture non plus, pour la même raison : mettre à jour pour de vrai relève d’une machine dédiée.',
    'tour.demo.win06': 'jg upgrade hyperf, complété en hyperfine, puis lancé — scoop remplace la 1.16.1 par la 1.20.0. Pris sur Windows, contre un vrai gestionnaire.',
    'tour.demo.omarchy06': 'Même geste sur Arch. À noter que yay pilote et que pacman ne fait que lire, ce qui explique que <span class="mono">jg install --pm pacman</span> n’existe pas tant que yay est installé. Pas de capture non plus, pour la même raison : mettre à jour pour de vrai relève d’une machine dédiée.',
    'tour.s10.h2': 'Pas seulement les gestionnaires de paquets.',
    'tour.s10.lede': 'Tapez <span class="mono">ssh</span> et une espace : la fenêtre propose les hôtes de votre <span class="mono">~/.ssh/config</span>, chacun avec son <span class="mono">HostName</span> en regard. Même cadre, mêmes touches — seul le catalogue a changé. jigger n’ouvre jamais la connexion ; il complète la ligne que vous lancerez vous-même.',
    'tour.demo.mac03': 'ssh et une espace : les serveurs de votre ~/.ssh/config, chacun avec son adresse en regard. Pris sur macOS.',
    'tour.demo.win03': 'La même liste et le même ~/.ssh/config, sous PowerShell — atelier compris, qui vient d’un Include. Pris sur Windows.',
    'tour.demo.omarchy03': 'Exactement la même liste, sur Arch — le sélecteur lit <span class="mono">~/.ssh/config</span> et rien d’autre, si bien que rien là-dedans n’est propre à une plateforme. Pris sur Omarchy.',
    'tour.s10.more': '<a href="/ssh.html">Le sélecteur SSH a sa propre page <span class="arw">→</span></a> — ce qu’il complète, ce qu’il ne lit jamais, et pourquoi un motif n’est pas un serveur.',
    'tour.s11.h2': 'Sans la fenêtre : appeler <span class="mono">jigger</span> soi-même.',
    'tour.s11.lede': 'Le greffon n’est qu’un client. Chaque sous-commande fonctionne toute seule — c’est la façon la plus claire de voir ce qui se passe quand quelque chose cloche, et la seule quand il n’y a aucun shell interactif : un script, une tâche planifiée, une CI.',
    'tour.s11.complete': 'Les candidats, un par ligne — exactement ce que la fenêtre aurait dessiné.',
    'tour.s11.render': 'Un cadre de fenêtre, dessiné une fois sur la sortie standard, métadonnées comprises.',
    'tour.s11.pick': 'Le sélecteur plein écran — celui-là même que <span class="mono">JIGGER_LIVE=0</span> vous donne sur <kbd>⇥</kbd> si vous préférez que rien n’apparaisse jamais sans qu’on le demande.',
    'tour.s11.prompt': 'L’état en cache que lit le hook d’invite — combien de mises à jour attendent, sans un seul appel réseau.',
    'tour.s11.warm': 'Reconstruit les catalogues. Lent, et bon à connaître le jour où le cadre affiche « catalogue en préparation… » et ne s’arrête plus.',
    'tour.s11.select': 'Et pour choisir des lignes <em>dans</em> un tube, il faut le demander explicitement — <span class="mono">--select</span> dessine la vue sur le terminal et n’envoie dans le tube que les noms retenus, un par ligne :',
    'tour.s11.pipe': 'Rien ne change quand la sortie n’est pas un terminal : <span class="mono">jg list | grep fd</span> affiche la même table brute qu’avant, octet pour octet, et <span class="mono">--json</span> n’est jamais paginé — c’est un contrat pour machine.',
    'tour.end.h2': 'Et c’est tout.',
    'tour.end.p': 'Onze étapes, et plus rien à apprendre : à partir d’ici, vous tapez ce que vous tapiez déjà, et le cadre suit. Le reste est du détail — les douze verbes et leurs traductions, les réglages, les pièges d’un profil PowerShell qui reprend <kbd>^R</kbd>.',
    'tour.end.use': 'La page de référence <span class="arw">→</span>',
    'tour.end.ssh': 'Le sélecteur SSH <span class="arw">→</span>',
    'tour.end.guide': 'Le guide complet <span class="arw">→</span>',
    /* --- page « utiliser » --- */
    ...
    'use.demo.omarchy01': 'yay -S visual-studio — les dépôts et l’AUR dans une seule liste : <span class="mono">◆</span> pour un paquet des dépôts, <span class="mono">▣</span> pour un paquet de l’AUR, <span class="mono">●</span> pour ce qui est déjà installé. Pris sur Omarchy.',
    ...
    'use.demo.omarchy02': 'jg install fd — avec un détail qui appartient à Arch : pacman et yay sont deux portes sur la même base, aussi <span class="mono">jg</span> liste-t-il vos paquets <strong>une</strong> fois, jamais deux. Pris sur Omarchy.',
    ...
    'use.regex.omarchy': 'yay -S fire liste les noms qui commencent par fire. ^R passe en regex, et les noms qui portent fire au milieu les rejoignent — la liste s’élargit avant de se resserrer. Puis (bird|fly) garde les firebird et les firefly, dépôts et AUR à la fois. Pris sur Omarchy.',
    ...
    'use.install.mac': 'L’installation par la façade se comporte pareil sur macOS, avec brew qui répond — et aucune capture n’en est enregistrée : en filmer une installerait des paquets pour de vrai sur la machine qui filme, ce que l’enregistrement Windows fait sur une machine virtuelle dédiée. Basculez sur Windows ci-dessus pour la voir.',
    'use.install.omarchy': 'Pareil sur Arch, avec yay qui répond — et ses propres questions relayées telles quelles, celles de l’AUR comprises, puisque jigger ne fait plus rien du tout une fois la ligne lancée. Aucune capture n’en est enregistrée : en filmer une demande d’installer pour de vrai, ce que l’enregistrement Windows fait sur une machine virtuelle dédiée. Passez à Windows ci-dessus pour la voir.',
    ...
    'use.upgrade.mac': 'C’est pareil sur macOS, où c’est brew qui met à jour — pas de capture non plus, pour la même raison : mettre à jour pour de vrai relève d’une machine dédiée. Basculez sur Windows ci-dessus pour la voir.',
    'use.upgrade.omarchy': 'Même geste sur Arch. À noter que yay pilote et que pacman ne fait que lire, ce qui explique que <span class="mono">jg install --pm pacman</span> n’existe pas tant que yay est installé. Pas de capture non plus, pour la même raison : mettre à jour pour de vrai relève d’une machine dédiée. Passez à Windows ci-dessus pour la voir.',
    ...
    'use.linux.p': 'Arch et Omarchy fonctionnent déjà — pacman et yay, même fenêtre, mêmes verbes, et les enregistrements ci-dessus y ont été pris eux aussi. Choisissez <strong>Omarchy</strong> dans l’en-tête et cette page bascule sur leurs commandes de bout en bout. Le guide d’installation va plus loin.',
    ...
    'ssh.action.lede': 'Les enregistrements ci-dessous lisent le même <span class="mono">~/.ssh/config</span> — un fixture de serveurs inventés, identique d’un système à l’autre. Rien, dans SSH, n’est propre à une plateforme, et c’est pourquoi les trois montrent exactement la même liste.',
    ...
    'ssh.demo.omarchy': 'La même liste encore, sur Arch : le sélecteur lit <span class="mono">~/.ssh/config</span> et rien d’autre, rien n’y est donc propre à la plateforme. Pris sur Omarchy.',
    ...
    'home.demo.omarchy': 'yay -S visual-studio — un cadre pour deux catalogues : <span class="mono">◆</span> un paquet des dépôts, <span class="mono">▣</span> un paquet de l’AUR, <span class="mono">●</span> ce qui est déjà installé. Pris sur Omarchy.',
  ...
  /* Le <title> de chaque page porte son propre `data-i18n` et suit donc le même
     ...
     l'appliquait aux quatre pages — onglets, favoris et indexation confondus. */
